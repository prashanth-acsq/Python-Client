...
def main():
    ...
    if args_6 in sys.argv: rtype = sys.argv[sys.argv.index(args_6) + 1]
    ...
    response = requests.request(method="POST", url=f"{base_url}/resize?size={size}&kar={kar}&fill={fill}&rtype={rtype}", files=files, verify=False)
    if response.status_code == 200:
        if rtype == "json":
            u.show_image(image=u.decode_image(response.json()["imageData"]))
        else:
            u.show_image(image=u.decode_binary_data_to_image(response.content))
    else:
